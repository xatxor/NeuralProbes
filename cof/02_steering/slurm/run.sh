...
# the benchmark, the concepts and the strengths are chosen here.
#
#   sbatch cof/02_steering/slurm/run.sh
#   BENCHMARK=math_500 CONCEPTS=657 ALPHAS=2.0 sbatch --export=ALL cof/02_steering/slurm/run.sh
#
...
BATCH=${BATCH:-4}
# Which benchmark to run. Every paired comparison is only as precise as the number of
# questions behind it, so more questions is what a tighter accuracy interval comes from.
...
WORKERS=${SLURM_ARRAY_TASK_COUNT:-8}
