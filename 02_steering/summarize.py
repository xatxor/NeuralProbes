...
def summarize(frame: pd.DataFrame) -> tuple[pd.DataFrame, pd.DataFrame]:
    baseline = frame[frame.alpha == 0.0].set_index(["benchmark", "id"])
    summary_rows = []
    for benchmark, group in frame.groupby("benchmark"):
        summary_rows.append(
            {"benchmark": benchmark, "concept_pair": None, "concept": "baseline", "layer": None, "alpha": 0.0, **metric_row(group[group.alpha == 0.0])}
        )
    effects = []
    ...
    for benchmark, rows in frame.groupby("benchmark"):
        ...
        for axis, (pair, concept) in zip(axes.flat, concepts):
            ...
            for layer, layer_rows in subset.groupby("layer"):
                ...
                points.sort()
                axis.plot([point[0] for point in points], [point[1] for point in points], marker="o", label=f"L{int(layer)}")
                limited = [point for point in points if point[2]]
                if limited:
                    axis.scatter([point[0] for point in limited], [point[1] for point in limited], marker="x", s=70, color="black")
        ...
        for axis in axes.flat[len(concepts):]:
            axis.remove()
        figure.colorbar(image, ax=list(axes.flat[:len(concepts)]), label="Accuracy (%)", shrink=0.7)
        figure.suptitle(f"{benchmark}: correctness by concept, layer, and alpha")
        figure.subplots_adjust(top=0.93, bottom=0.07, left=0.08, right=0.9, hspace=0.55, wspace=0.25)
...
def main() -> None:
    ...
    columns = (
        "key", "benchmark", "id", "concept_pair", "concept", "layer", "alpha",
